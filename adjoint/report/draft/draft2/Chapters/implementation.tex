...
The function evaluation consists of solving the state equation, which requires $\mathcal{O}(n)$ operations, and then applying the functional on the control and the state on all $n+1$ time points, which we assume at most requires $\mathcal{O}(n)$ operations. We can therefore assume that the sequential objective function evaluation execution time is 
\begin{align*}
T_s = \mathcal{O}(n)
\end{align*}
