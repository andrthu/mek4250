...
\begin{align*}
&J(y,v) = \frac{1}{2}\int_0^{100}v(t)^2dt + \frac{1}{2}(y(T)-11.5)^2 \\
&\left\{
     \begin{array}{lr}
       	y'(t)=-0.097y(t) + v(t) \ t\in(0,1)\\
       	y(0)=3.2
     \end{array}
   \right. 
\end{align*}
We have chosen $T=100$, so that the small time steps $\Delta t$ does not get to small, when $n$ is large. We test the algorithm for $n=600000,1200000$ number of time steps. 
