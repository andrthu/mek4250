...
\begin{align}
\forall \ i,0\leq i\leq N-1 \quad |\lambda_i^k-y(T_i)| + \max_{t\in[T_{i},T^{i+1}]}|y_{i+1}^k(t)-y(t)| \leq c_k\Delta T^k \label{P_error_bound}
\end{align}
\end{theorem}
It is important to note that the error bound given in the above theorem only holds for fixed $k$s, since the constant $c_k$ grows with $k$. This means that for a fixed number of iterations $k$ of the Parareal algorithm converges to the solution with $\Delta T$ at a rate of $\bold O(\Delta T^k)$. In \cite{lions2001resolution}, they used a first order method for their coarse propagator. It turns out that if one instead uses a scheme of order $p$, the Parareal algorithm behaves as numerical method with error bound $\bold O(\Delta T^{pk})$ after $k$ iterations. This was shown in \cite{bal2005convergence}, where the bounds were derived for more general equations. 
\\
\\
The case where we let $\Delta T$ be fixed, and look at convergence when we increase $k$, is analysed in \cite{gander2007analysis}. Here the authors again investigate the convergence of the equation (\ref{convergence_ODE}). They found that the convergence was super linear in $k$ for bounded time intervals $[0,T]$, and linear for a unbounded time interval. 