...
In this section we look at some of the convergence properties of the Parareal algorithm given in the literature. The first publication on Parareal \cite{lions2001resolution} studied the convergence in ontext of the following equation: 
\begin{align}
\frac{\partial}{\partial t} y(t)=ay(t),\quad t\in [0,T],\quad y(0)=y_0\label{convergence_ODE}
\end{align}
We stae their findings in the theorem below:
\begin{theorem}
...
\begin{align}
\forall \ i,0\leq i\leq N-1 \quad |\lambda_i^k-y(T_i)| + \max_{t\in[T_{i},T^{i+1}]}|y_{i+1}^k(t)-y(t)| \leq c_k\Delta T^k
\end{align}
\end{theorem}
The bound given in the above theorem is for fixed $k$ values, which means that doing $k$ iterations of the Parareal algorithm can be thought of as a numerical method of order $O(\Delta T^k)$.
