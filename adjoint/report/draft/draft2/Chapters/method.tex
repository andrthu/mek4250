...
\end{align*}
Where $\bold F_{\Delta T}$ is the function defined in (\ref{F_operator}). In chapter 2 I explained how this could be solved by setting $\lambda_i= \bold F_{\Delta T}(\lambda_{i-1}$, which is the same as solving $\hat{J}(\Lambda)=0$. This eventually led us to the parareal scheme, defined on matrix form as:
\begin{align*}
