...
In order for us to make it easier to differentiate the gradient of the reduced objective function (\ref{reduced_viritual}), we try to write up $\hat{\bold J}$ on vector notation. Let us first define the vector $x\in\mathbb{R}^{N-1}$ as:
\begin{align}
x= \left( \begin{array}{c}  
   ...
   \end{array}  \right)
\end{align} 
We can then formulate the problem (\ref{reduced_viritual}) as:
\begin{align}
\min_{\Lambda}\hat{\bold J}(\Lambda) = x^Tx \label{vector_J}  
\end{align}
Here $x^T$ means $x$ transposed. We then want to express $x$ in terms of $\Lambda$ and $y_0$. To do this, we first find an expression for $\{y_i(T_i)\}_{i=1}^{N-1}$:
\begin{align}
\left( \begin{array}{c}
   y_0(T_1) \\  
   y_1(T_2) \\ 
...
\begin{align*}
\nabla\hat{J}(\Lambda) = 2 M^TM\Lambda - 2M^Tb
\end{align*}
...
||Mx||^2=0 \iff x=&0
\end{align*}
The second requirement for positive definiteness 
\\
\\
As mentioned above, \cite{maday2002parareal} solves the problem (\ref{decomp_E}-\ref{penalty_func}), using the gradient method:
\begin{align}
