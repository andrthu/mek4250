...
We want to solve reducible optimization problems of type (\ref{OCP_DEF_J}-\ref{OCP_DEF_E}), where the state equation constraint $E(y(t),v,y_0)=0$ is time dependent and solved on the interval $I=[0,T]$, with initial condition $y_0$. To introduce parallelism to our optimal control problem, we need to decompose the time domain and the state equation. 
\begin{definition}[Decomposed state equation] \label{DecompEDef}
Let $0=T_0<T_1<\cdots<T_{N-1}<T_N=T$ and define the $i$-th decomposed subinterval to be $I_i=[T_{i-1},T_i]$. We then introduce $N-1$ intermediate initial conditions $\Lambda=(\lambda_1,...,\lambda_{N-1})$, and set $\lambda_0=y_0$. Using the these intermediate initial conditions we are able to define $N$ decomposed state equations:
\begin{align}
E^i(y_i(t),v,\lambda_{i-1})= 0 \quad t\in I_i. \label{DecompState}
\end{align} 
The state $y(t)$ that lives on the entire time interval is defined as:
\begin{align}
 y(t)=\left\{
     \begin{array}{lr}
...
\end{align}
We will continue to let $E(y(t),v)=0$ denote the undecomposed state equation for the entire interval. $E(y(t),v,\Lambda)=0$ represents the decomposed state equation defined by solving the $N$ decomposed equations (\ref{DecompState}) for the $y_i$s that make up $y(t)$ in (\ref{GatherState}).  
\end{definition}
\noindent
Using the decomposed time interval, state and state equation of definition \ref{DecompEDef}, we can define the decomposed optimal control problem. Because we want the state to be continuous, we need to introduce new constraints on the problem that enforces the continuity of $y(t)$. These new constraints are written up along with the decomposed reformulation of the optimal control problem in definition \ref{DecompOCPDef}.
...
\textrm{subject to:} \ &E^i(y_i(t),v,\lambda_{i-1})=0, \quad t\in [T_{i-1},T_i], \quad i=1,...,N. \label{decomposed problem}
\end{align}
...
\hat J(v) = J(y(v)(t),v)
\end{align*}
The decomposed and reduced optimal control problem with time dependent differential constraints is then the following constrained minimization problem:
\begin{align}
