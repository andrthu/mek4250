...
\end{align*}
What separates different line search methods, is how one chooses direction $p_k$ and step length $\alpha_k$.
\\
\\
The steepest descent method is the simplest line search method that exists, and is  
\begin{align*}
x^{k+1} = x^k - \alpha \nabla J(x^k)
\end{align*}  
