...
\end{align}
A point $\bar{v}\in V$ is called a local minimizer of if there exists a neighbourhood $\mathcal{N}\subset V$ of $\bar{v}$ such that:
\begin{align}
...
\subsection{BFGS Based Algorithm for Solving the Reduced Optimization Problem}
We end this chapter by formally writing up an algorithm that numerically solves the reduced optimization problem (\ref{reduced problem}) with the BFGS method. We will use this algorithm to measure the performance of the parallel method that we will propose in chapter \ref{method_chap}, and when we refer to the sequential or serial algorithm we will mean algorithm \ref{SEQ_ALG}.
\\
\begin{algorithm}[H] 
\KwData{Choose initial control $v^0$ and tolerance $\tau$}
\While{$||\hat J'(v^k)||\geq \tau$ }{
$v^{k+1} \leftarrow v^k - \alpha^k H^k \hat J'(v^k) $\;
Update $H^{k+1}$\;
}
\caption{BFGS method\label{SEQ_ALG}}
\end{algorithm}
\noindent
\\
Notice that to apply algorithm \ref{SEQ_ALG}, we need a discretized expression for $\hat J(v^k)$ and $\hat J'(v^k)$. We will derive such expressions for the example problem in chapter \ref{disc_chap}.
