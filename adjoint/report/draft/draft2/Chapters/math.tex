...
\begin{definition}
The Crank-Nicolson finite difference scheme on equation (\ref{general_1st_ODE}) is defined by the following formula for $y_k$:
\begin{align}
...
\end{align}
Other more accurate finite difference schemes exist, but in this thesis restrict the usage of finite difference methods to the ones presented in this section. 
...
\begin{align}
S_l = \Delta t\sum_{k=0}^{n-1} v_k
\end{align}
\begin{align}
S_r = \Delta t\sum_{k=1}^{n} v_k
\end{align}
\begin{align}
S_{trap} = \Delta t\frac{v_0+v_n}{2}+\Delta t\sum_{k=1}^{n-1} v_k
\end{align}
\end{definition}
