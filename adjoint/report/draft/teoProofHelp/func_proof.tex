...
E_u^*p = J_u
\end{align*}
As always, $E$ refers to the state equation, which in our case is Burgers equation (\ref{burger}). I will now write up the weak formulation of (\ref{burger}), so that I can use it to derive the adjoint. Let $(\cdot,\cdot)$ be $L^2$ inner product over $\Omega$.
\begin{align*}
