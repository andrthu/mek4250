...
\end{align*} 
Where p is the solution of the adjoint equation $E_y^*p=J_y$, and $E$ is the collection of the interval specific state equations $E^i$. Lets do as we did for non-penalty approach, and write up the weak formulation of the state equations:
\begin{gather*}
L^i[y,\phi] = \int_{T_{i-1}}^{T_{i}}y_i(t)\phi'(t)+\alpha y_i(t)\phi(t)dt +\lambda_{i-1}\phi(T_{i-1})=- \int_{T_{i-1}}^{T_{i}}u(t)\phi(t)=R^i[u,\phi]\\ \forall \ \phi \in C^{\infty}_c([T_{i-1},T_{i}))
\end{gather*}
\begin{align*}
